...
def generate_mapwithlist():
    ...
    hos_map.save('templates/hos1map.html')

    # MAP END

    # List Ka Code
    li2.sort(key=lambda x: x["Distance"])
    tbl = "<tr><td>Name</td><td>Distance</td></tr>"
    c = 0
    for y in li2:
        if c == 16:
            break
        ...
        c = c + 1
        a = "<tr onclick=genMap2();><td class='hfn'>%s</td>" % y['Name']
    ...
    %s
        ...
        <script>
            ...
            .then(()=>window.location.href="info2.html")
                        }
            </script>
    </body>
    </html>
